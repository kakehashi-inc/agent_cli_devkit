...
Agent CLI DevKit は、AI エージェント CLI（Claude Code / Codex CLI など）の開発環境を 1 つのアプリで管理する Electron 製デスクトップツールです。Claude Developer Tool と Codex Developer Tool の全機能を統合し、エージェント（Claude / Codex）ごとにグループ化された UI で提供します。
...
ダッシュボードとタイトルバーのナビゲーションは Claude / Codex のグループ単位で全機能を表示します。内部構造もエージェント単位でファイル分割されており、新しい agent CLI を追加しやすい構成になっています（詳細は `Documents/システム仕様.md` を参照）。
...
│   └── agents/              #   agent 別ブリッジ (claude.ts / codex.ts)
├── renderer/                # React + MUI UI
│   ├── agents/              #   agent 別画面 (claude/ codex/) と registry
│   ├── components/          #   共通コンポーネント (TitleBar/Dashboard 等)
│   └── i18n/locales/        #   ロケール (ja/ en/ × app/claude/codex)
├── shared/                  # 型定義・定数
│   └── agents/              #   agent 共通型と agent 別定数/型 (claude/ codex/)
└── public/                  # アイコン等
```
