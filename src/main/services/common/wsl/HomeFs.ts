...
/**
 * AgentEnvironment（native / wsl）に対するファイル操作を抽象化する。
 *
 ...
 *
 * relPath はすべて Linux ホームからの相対パス（例: '.claude.json', '.codex/config.toml'）。
 */
export class HomeFs {
    ...
    constructor(env: AgentEnvironment, detector: WslDetector) {
        this.env = env;
