...
import { GROK_CONFIG_REL, GROK_WSL_CLI_TEST, SETTINGS_FIELDS } from '../../../shared/agents/grok/constants';
import {
    GrokEnvironment,
    OSType,
    SettingsFieldSpec,
 ...
 */
export class GrokConfigManager {
     ...
     */
    async read(env: GrokEnvironment): Promise<SettingsReadResult> {
     ...
     */
    async write(env: GrokEnvironment, values: SettingsValues): Promise<SettingsWriteResult> {
        const fs = this.fsFor(env);
        ...
        try {
            await fs.writeText(GROK_CONFIG_REL, text);
            return { ok: true };
        } catch (error) {
     ...
     */
    async writeRaw(env: GrokEnvironment, raw: string): Promise<SettingsWriteResult> {
        try {
            parse(raw);
        } catch {
        ...
        try {
            await fs.writeText(GROK_CONFIG_REL, content);
            return { ok: true };
        } catch (error) {
