...
import { CLAUDE_ASSET_CHANNELS } from '../../../shared/agents/claude/constants';
import { AssetKind, ClaudeEnvironment } from '../../../shared/agents/claude/types';
 ...
 */
export function registerClaudeAssetHandlers(
    manager: ClaudeAssetManager,
    getMainWindow: () => BrowserWindow | null
): void {
    // 環境一覧（native + Claude 入り WSL distro）を取得
    ipcMain.handle(CLAUDE_ASSET_CHANNELS.GET_ENVIRONMENTS, () => {
        return manager.getEnvironments();
    });

    // 指定環境・種別のサブディレクトリ一覧を取得
    ipcMain.handle(CLAUDE_ASSET_CHANNELS.LIST, (_, env: ClaudeEnvironment, kind: AssetKind) => {
    ...
    ipcMain.handle(CLAUDE_ASSET_CHANNELS.INSPECT_UPLOAD, (_, env: ClaudeEnvironment, kind: AssetKind) => {
        return manager.inspectUpload(env, kind, getMainWindow());
    });

    // ZIP を対象フォルダへ展開（overwrite 時は同名を丸ごと置換）
    ipcMain.handle(
        CLAUDE_ASSET_CHANNELS.UPLOAD,
    ...
    ipcMain.handle(
        CLAUDE_ASSET_CHANNELS.UPLOAD_MD,
    ...
    // 選択したエージェント / スキルを削除
    ipcMain.handle(CLAUDE_ASSET_CHANNELS.DELETE, (_, env: ClaudeEnvironment, kind: AssetKind, relPaths: string[]) => {
        return manager.deleteSelected(env, kind, relPaths);
    });

    // git が利用可能か（公式スキルインポートボタンの活性判定）
    ipcMain.handle(CLAUDE_ASSET_CHANNELS.IS_GIT_AVAILABLE, () => {
        return manager.isGitAvailable();
    });

    // 公式スキルリポジトリを clone/更新し、スキル一覧を返す
    ipcMain.handle(CLAUDE_ASSET_CHANNELS.LIST_OFFICIAL_SKILLS, () => {
        return manager.listOfficialSkills();
    });

    // 選択した公式スキルを対象環境へ取り込む（公式同士は確認なしで置換）
    ipcMain.handle(CLAUDE_ASSET_CHANNELS.IMPORT_OFFICIAL_SKILLS, (_, env: ClaudeEnvironment, relPaths: string[]) => {
