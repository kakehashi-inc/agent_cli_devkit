...
export const UpdateNotification: React.FC = () => {
    ...
    useEffect(() => {
        const unsubscribe = window.agentCliDevkit.updater.onStateChanged(next => {
            setState(next);
    ...
    const handleUpdate = (): void => {
        void window.agentCliDevkit.updater.download();
    };
        ...
        if (state.status === 'downloading') {
            ...
            return (
                <Stack spacing={1} sx={{ minWidth: 280 }}>
                    <Typography variant='body2'>{t('updater.downloading', { progress: percent })}</Typography>
    ...
    return (
        <Snackbar open={isVisible} anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }} autoHideDuration={null}>
            <Box
                sx={{
