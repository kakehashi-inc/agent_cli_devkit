 ...
 * 各セクションが Grok（~/.grok）と その他のツール（~/.serena 等）の両方をまとめて扱う。
 */
export const Cleanup: React.FC = () => {
    ...
    const load = async () => {
        setLoading(true);
        try {
            const [grokEnvs, otherEnvs] = await Promise.all([
                window.agentCliDevkit.grok.cleanup.getEnvironments(),
                window.agentCliDevkit.grok.cleanup.getOtherEnvironments(),
            ]);
            // Grok を持つ環境と Serena 等を持つ環境の和集合（envId で重複排除）
            const merged = new Map<string, { env: GrokEnvironment; label: string }>();
            for (const e of [...grokEnvs, ...otherEnvs]) {
            ...
            <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 2 }}>
                <Box>
                    <Typography variant='h4' component='h1' sx={{ mb: 1 }}>
                        {t('grok.cleanup.title')}
                    </Typography>
                    <Typography variant='body2' color='text.secondary'>
                        {t('grok.cleanup.description')}
                    </Typography>
                </Box>
                <Button
                    variant='outlined'
            ...
            {wslEnvs.length > 0 && (
                <>
                    ...
                    {wslEnvs.map(({ env, label }) => (
                        <Box key={envId(env)}>
                            <Typography variant='h6' sx={{ mb: 1 }}>
                                {t('grok.cleanup.wslSection', { distro: label })}
                            </Typography>
                            <PlatformCleanupSection env={env} label={label} onNotify={notify} />
                        </Box>
                    ))}
