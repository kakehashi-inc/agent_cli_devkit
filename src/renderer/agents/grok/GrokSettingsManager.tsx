...
export const GrokSettingsManager: React.FC = () => {
    ...
    const load = async () => {
        setLoading(true);
        try {
            const envs = await window.agentCliDevkit.grok.config.getEnvironments();
            ...
            <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 2 }}>
                <Box>
                    <Typography variant='h4' component='h1' sx={{ mb: 1 }}>
                        {t('grok.settings.title')}
                    </Typography>
                    <Typography variant='body2' color='text.secondary'>
                        {t('grok.settings.description')}
                    </Typography>
                </Box>
                <Button
                    variant='outlined'
            ...
            {wslEnvs.length > 0 && (
                <>
                    ...
                    {wslEnvs.map(({ env, label }) => (
                        <Box key={envId(env)}>
                            <Typography variant='h6' sx={{ mb: 1 }}>
                                {t('grok.settings.wslSection', { distro: label })}
                            </Typography>
                            <SettingsSection env={env} onNotify={notify} />
                        </Box>
                    ))}
