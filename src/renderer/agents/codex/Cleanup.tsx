 ...
 * 各セクションが Codex（~/.codex）と その他のツール（~/.serena 等）の両方をまとめて扱う。
 */
export const Cleanup: React.FC = () => {
    ...
    const load = async () => {
        setLoading(true);
        try {
            const [codexEnvs, otherEnvs] = await Promise.all([
                window.agentCliDevkit.codex.cleanup.getEnvironments(),
                window.agentCliDevkit.codex.cleanup.getOtherEnvironments(),
            ]);
            // Codex を持つ環境と Serena 等を持つ環境の和集合（envId で重複排除）
            const merged = new Map<string, { env: CodexEnvironment; label: string }>();
            for (const e of [...codexEnvs, ...otherEnvs]) {
            ...
            <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 2 }}>
                <Box>
                    <Typography variant='h4' component='h1' sx={{ mb: 1 }}>
                        {t('codex.cleanup.title')}
                    </Typography>
                    <Typography variant='body2' color='text.secondary'>
                        {t('codex.cleanup.description')}
                    </Typography>
                </Box>
                <Button
                    variant='outlined'
            ...
            {wslEnvs.length > 0 && (
                <>
                    ...
                    {wslEnvs.map(({ env, label }) => (
                        <Box key={envId(env)}>
                            <Typography variant='h6' sx={{ mb: 1 }}>
                                {t('codex.cleanup.wslSection', { distro: label })}
                            </Typography>
                            <PlatformCleanupSection env={env} label={label} onNotify={notify} />
                        </Box>
                    ))}
