...
import type { ClaudeCodeEnvInfo } from '@shared/agents/claude/types';
...
export const ClaudeCodeManager: React.FC = () => {
    ...
    const load = async () => {
        setLoading(true);
        try {
            const envs = await window.agentCliDevkit.claude.code.getEnvironments();
            ...
            {wslEnvs.length > 0 && (
                <>
                    ...
                    {wslEnvs.map(info => (
                        <Box key={envId(info.env)}>
                            <Typography variant='h6' sx={{ mb: 1 }}>
                                {t('claude.claudeCode.wslSection', { distro: info.label })}
                            </Typography>
                            <ClaudeEnvMcpSection info={info} onNotify={notify} />
                        </Box>
                    ))}
