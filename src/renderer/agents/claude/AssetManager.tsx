...
export const AssetManager: React.FC = () => {
    ...
    const load = async () => {
        setLoading(true);
        try {
            const envs = await window.agentCliDevkit.claude.asset.getEnvironments();
            ...
            <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 2 }}>
                <Box>
                    <Typography variant='h4' component='h1' sx={{ mb: 1 }}>
                        {t('claude.assetManager.title')}
                    </Typography>
                    <Typography variant='body2' color='text.secondary'>
                        {t('claude.assetManager.description')}
                    </Typography>
                </Box>
                <Button
                    variant='outlined'
            ...
            {wslEnvs.length > 0 && (
                <>
                    ...
                    {wslEnvs.map(({ env, label }) => (
                        <Box key={envId(env)}>
                            <Typography variant='h6' sx={{ mb: 1 }}>
                                {t('claude.assetManager.wslSection', { distro: label })}
                            </Typography>
                            <AssetManagerSection env={env} onNotify={notify} />
                        </Box>
                    ))}
