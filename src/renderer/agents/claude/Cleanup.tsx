    ...
    const load = async () => {
        ...
        try {
            const [claudeEnvs, otherEnvs] = await Promise.all([
                window.agentCliDevkit.claude.cleanup.getEnvironments(),
                window.agentCliDevkit.claude.cleanup.getOtherEnvironments(),
            ]);
            ...
            <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 2 }}>
                <Box>
                    <Typography variant='h4' component='h1' sx={{ mb: 1 }}>
                        {t('claude.cleanup.title')}
                    </Typography>
                    <Typography variant='body2' color='text.secondary'>
                        {t('claude.cleanup.description')}
                    </Typography>
                </Box>
                <Button
                    variant='outlined'
            ...
            {wslEnvs.length > 0 && (
                <>
                    ...
                    {wslEnvs.map(({ env, label }) => (
                        <Box key={envId(env)}>
                            <Typography variant='h6' sx={{ mb: 1 }}>
                                {t('claude.cleanup.wslSection', { distro: label })}
                            </Typography>
                            <PlatformCleanupSection env={env} label={label} onNotify={notify} />
                        </Box>
                    ))}
