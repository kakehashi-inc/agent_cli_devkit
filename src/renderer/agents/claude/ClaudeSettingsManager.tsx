...
export const ClaudeSettingsManager: React.FC = () => {
    ...
    const load = async () => {
        setLoading(true);
        try {
            const envs = await window.agentCliDevkit.claude.settings.getEnvironments();
            ...
            {wslEnvs.length > 0 && (
                <>
                    ...
                    {wslEnvs.map(({ env, label }) => (
                        <Box key={envId(env)}>
                            <Typography variant='h6' sx={{ mb: 1 }}>
                                {t('claude.settings.wslSection', { distro: label })}
                            </Typography>
                            <SettingsSection env={env} onNotify={notify} />
                        </Box>
                    ))}
